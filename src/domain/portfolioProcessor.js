...
export class PortfolioProcessor {
    ...
    static processPortfolio = ( positions, calculatedData, referenceData, factors ) => {
        ...
        //factors
        const portData = candles.map( a => a.close );
        calculatedData.factors = [];
        for ( const index of factors ) {
            //TODO: make point-to-point correspondence correctly
            if ( index.history ) {
                let indexData = index.history.candles.map( a => a.close );
                const len = indexData.length - portData.length;
                if ( len > 0 ) {
                    indexData.splice( 0, len );
                }
                if ( indexData.length === portData.length && portData.length > 1 ) {
                    calculatedData.factors.push( {
                        id: index.securityId,
                        correlation: statProcessor.sampleCorrelation( indexData, portData )
                    }
                    );
                }
            }
